  ...
  return (
    ...
    >
      ...
      <div className="p-1 space-y-1">
        ...
        <button
          onClick={onReset}
          className="w-full flex items-center gap-3 px-3 py-2 text-[var(--color-red)] hover:bg-white/10 hover:text-red-400 transition-colors text-xs font-bold font-mono text-left group border-t border-gray-800 mt-1 pt-2"
        >
          <RotateCcw
            size={14}
      ...
      <div className="space-y-1 text-sm md:text-base font-bold">
        {lines.map((line, i) => (
          <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <span style={{ color: COLORS.RED }} className="mr-2">
...
const ShutdownScreen = ({ onReboot }) => (
  ...
  >
    <div className="text-[var(--color-red)] text-4xl font-black tracking-widest mb-4">
      SYSTEM HALTED
    </div>
    ...
    >
      <div className="bg-black/60 backdrop-blur-sm border border-red-900/50 p-4 relative overflow-hidden rounded-lg hover:border-red-500 transition-colors">
        ...
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-black text-[var(--color-red)] tracking-widest">
            SYNC_CHRONO
          </span>
        ...
        <div className="flex items-end gap-2 mb-2">
          <Clock
            size={24}
            className="text-black mb-1 bg-[var(--color-yellow)] rounded-full p-0.5"
          />
          <span
            className="text-4xl font-black text-white tracking-tighter"
            style={{ textShadow: `2px 2px 0px ${COLORS.RED}` }}
          >
            {gregTime}
          </span>
        </div>
        ...
        <div
          className="absolute top-0 left-0 w-full h-1 bg-[var(--color-red)]/20 animate-scanline"
      ...
      >
        ...
        {status === 'ANALYZING' && (
          <div className="text-center w-full">
            ...
            <div className="text-xs font-black text-[var(--color-blue)] animate-pulse">
              NEURAL_SCANNING...
            </div>
        ...
        {status === 'SUCCESS' && (
          <div className="text-center">
            <div className="text-[var(--color-blue)] font-bold text-sm">UPLOAD_COMPLETE</div>
          </div>
        )}
        {status === 'ERROR' && (
          <div className="text-center">
            <div className="text-[var(--color-red)] font-bold text-sm">SCAN_FAILED</div>
          </div>
        )}
        {status === 'IDLE' && (
          <div className="text-center group-hover:scale-105 transition-transform pointer-events-none">
            <UploadCloud
              size={32}
              className="mx-auto mb-2 text-gray-500 group-hover:text-[var(--color-blue)]"
            />
          ...
          <div className="bg-black/95 backdrop-blur-xl p-1 h-full w-full" style={{ skewX: 5 }}>
            <div className="bg-black/80 p-6 border border-gray-800 relative overflow-hidden">
              ...
              <div className="space-y-2 relative z-10">
                {[
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 rounded group hover:bg-[var(--color-yellow)] transition-all duration-200"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-6 h-6 flex items-center justify-center bg-white/5 rounded group-hover:bg-black/20 transition-colors">
                        <app.icon
                          size={14}
                          className="text-[var(--color-blue)] group-hover:text-black"
                        />
                      </div>
                      <span className="font-bold text-gray-300 group-hover:text-black tracking-wide text-sm">
                        {app.label}
                      </span>
                    </div>
                    <ChevronRight
                      size={14}
                      className="text-[var(--color-red)] opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all font-bold"
                    />
                  </button>
                ))}
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-gray-800 relative z-10">
                <button
                  onClick={onShutdown}
                  className="w-full flex items-center gap-3 p-3 text-[var(--color-red)] hover:bg-[var(--color-red)]/10 hover:text-red-400 rounded transition-colors group"
                >
                  <Power
                    size={18}
                    className="group-hover:drop-shadow-[0_0_8px_rgba(255,0,0,0.8)]"
                  />
                  <span className="font-bold">JACK_OUT</span>
                </button>
              </div>
            </div>
          </div>
...
export default function WinOS() {
  ...
  const [financeData, setFinanceData] = useState({
    ...
    upcomingBills: [
      ...
      { id: 2, name: 'Gym', amount: 45.0, dueDate: '2024-08-05' },
    ],
    debts: [{ id: 1, name: 'Credit Card', amount: 2500, interestRate: 22.5 }],
    portfolio: {
  ...
  const logEvent = (event, type = 'info') => {
    setAuditLog((prev) =>
      [{ id: Date.now(), event, time: new Date().toLocaleTimeString(), type }, ...prev].slice(0, 50)
    );
  };
  ...
  const categorizeTransaction = (desc, amount) => {
    ...
    if (lowerDesc.includes('dinner') || lowerDesc.includes('restaurant'))
      return { category: 'Food/Drink' };
    if (lowerDesc.includes('groceries') || lowerDesc.includes('market'))
      return { category: 'Groceries' };
    if (lowerDesc.includes('coffee') || lowerDesc.includes('starbucks'))
      return { category: 'Coffee' };
    if (lowerDesc.includes('uber') || lowerDesc.includes('taxi') || lowerDesc.includes('transport'))
  ...
  useEffect(() => {
    ...
    if (lastTransaction) {
      ...
      if (lastTransaction.amount > 500 || hour < 6 || hour > 23) {
        addNotification(
          `Anomaly detected: ${lastTransaction.desc} for ${lastTransaction.amount}`,
          'error'
        );
      }
      ...
      setTimeout(() => {
        setFinanceData((prev) => ({
          ...
          recent: prev.recent.map((t) =>
            t.desc === lastTransaction.desc
              ? { ...t, enriched: true, logo: 'https://example.com/logo.png' }
              : t
          ),
        }));
      }, 1000);
  ...
  const apps = {
    ...
    settings: {
      ...
      props: {
        config: sysConfig,
        auditLog,
        onUpdateConfig: (k, v) => setSysConfig((prev) => ({ ...prev, [k]: v })),
      },
    },
  ...
  const openWindow = (appId, data = null) => {
    ...
    if (existing) {
      setWindows((prev) =>
        prev.map((w) => (w.id === appId ? { ...w, minimized: false, data: data || w.data } : w))
      );
      bringToFront(appId);
      return;
    }
  ...
  const handleTransactionUpdate = (newData) => {
    const { category, isRecurring, isTaxDeductible } = categorizeTransaction(
      newData.summary || '',
      newData.amount
    );

    setFinanceData((prev) => {
      const newSubs = isRecurring
        ? [
            ...prev.subscriptions,
            { service: newData.summary, amount: newData.amount, cycle: 'Monthly' },
          ]
        : prev.subscriptions;
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 bg-black flex flex-col items-center justify-center p-8 text-center font-mono text-[var(--color-red)] z-[9999]">
        ...
        <p className="text-sm border flex items-center gap-2 px-2 py-1 mb-8 border-red-900 bg-red-900/10">
          ERROR_CODE: VIEWPORT_TOO_SMALL
        </p>
        <p className="text-gray-500 text-xs">
          This Neural Interface requires a standard desktop resolution to function safely.
        </p>
  ...
  return (
    <div
      className="relative w-screen h-screen overflow-hidden font-sans bg-black selection:bg-[var(--color-yellow)] selection:text-black"
      onClick={() => {
        setStartMenuOpen(false);
        handleCloseContextMenu();
      }}
    ...
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-[10rem] font-black text-white/5 select-none pointer-events-none whitespace-nowrap z-0 glitch mix-blend-overlay"
          data-text="NIGHT CITY"
        >
          NIGHT CITY
        </div>
      </div>
      ...
      <div
        ref={desktopRef}
        className="relative z-10 w-full h-full p-6 pb-20"
        key={desktopKey}
        onClick={(e) => e.stopPropagation()}
      >
        {/* DRAGGABLE ICONS */}
        {desktopIcons.map((item) => {
          ...
          return (
            <DraggableItem
              key={item.id}
              initialX={item.x || '20px'}
              initialY={item.defaultY}
              className="w-24"
            ...
            >
              ...
              >
                <div className="w-16 h-16 bg-black/40 backdrop-blur-sm border border-gray-600 rounded-xl flex items-center justify-center group-hover:bg-[var(--color-yellow)] group-hover:border-[var(--color-yellow)] transition-all duration-200 shadow-lg group-hover:shadow-[0_0_20px_rgba(255,224,0,0.4)] relative overflow-hidden">
                  ...
                  <app.icon
                    size={30}
                    className="text-[var(--color-blue)] group-hover:text-black transition-colors"
                  />
                </div>
              ...
              >
                {Component ? (
                  ...
                  <div className="p-4 bg-black h-full">
                    <h3 className="text-xs font-bold mb-6 text-[var(--color-red)]">
                      /VAR/ROOT/USER/SHARDS
                    </h3>
                    <div className="flex gap-2 mb-4 border-b border-gray-800 pb-2">
                      <button
                        onClick={() => document.getElementById('hidden-upload').click()}
                        className="px-3 py-1 bg-[var(--color-yellow)] text-black text-xs font-bold rounded-sm hover:bg-white transition-colors"
                      >
                        IMPORT_SHARD
                      </button>
                      <input
                    ...
                    {files.length === 0 ? (
                      <div className="text-center mt-20 text-gray-600 font-mono">NO_DATA_FOUND</div>
                    ) : (
                      <div className="grid grid-cols-4 gap-4">
                        {files.map((f, i) => (
                          ...
                          >
                            <FileText
                              size={32}
                              className="text-[var(--color-blue)] group-hover:text-[var(--color-yellow)]"
                            />
                            <span className="text-xs text-center truncate w-full text-gray-400">
                              {f.name}
                            </span>
                          </div>
                        ))}
                  ...
                  <div className="flex items-center justify-center h-full bg-black min-h-[400px]">
                    {win.data ? (
                      <div className="text-center">
                        <div className="mb-4 text-[var(--color-blue)] font-mono">
                          DECRYPTED_SEGMENT__
                        </div>
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-[var(--color-red)] font-mono font-bold">
                          ENCRYPTED CONTENT
                        </p>
      ...
      {/* TASKBAR */}
      <div
        className="absolute bottom-0 left-0 right-0 h-12 bg-black/90 backdrop-blur-md border-t border-gray-800 flex items-center justify-between px-2 z-50 select-none"
        onClick={(e) => e.stopPropagation()}
      >
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  ...
                  // Search Apps
                  const foundApp = Object.keys(apps).find(
                    (key) => apps[key].name.toLowerCase().includes(term) || key.includes(term)
                  );
                  if (foundApp) {
                    openWindow(foundApp);
                    e.target.value = '';
        ...
        <div className="flex items-center gap-1">
          {windows.map((win) => {
            ...
            return (
              <button
                key={win.id}
                onClick={() => (win.minimized ? toggleMinimize(win.id) : bringToFront(win.id))}
              ...
              >
                <span
                  className={`text-xs font-bold uppercase ${activeWindowId === win.id ? 'text-[var(--color-yellow)]' : 'text-gray-500 group-hover:text-gray-300'}`}
                >
                  {appName}
                </span>
                {activeWindowId === win.id && (
                  <div className="absolute bottom-0 left-0 w-full h-[2px] bg-[var(--color-yellow)] shadow-[0_0_10px_var(--color-yellow)]" />
                )}
              </button>
            );
          })}
        ...
        <div className="flex items-center gap-6 pr-6 text-xs font-mono font-bold">
          ...
          <button
            onClick={() => setShowPersianDate(!showPersianDate)}
            className="flex items-center gap-2 text-[var(--color-blue)] hover:text-[var(--color-yellow)] transition-colors bg-white/5 px-3 py-1 rounded"
          >
            {showPersianDate ? (
              <span className="text-[var(--color-yellow)]">{getPersianDate()}</span>
            ) : (
              <span>
