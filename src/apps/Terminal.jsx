...
const TerminalApp = ({ financeData }) => {
  ...
  const handleCommand = (cmd) => {
    ...
    switch (cleanCmd) {
      case 'help':
        output = [
          'AVAILABLE DAEMONS:',
          '  balance  - Check current eddies',
          '  clear    - Flush terminal buffer',
          '  date     - System timestamp',
          '  whoami   - User identity',
          '  hack     - Run breach protocol simulation',
        ];
        break;
      case 'clear':
        setHistory([]);
      ...
      case 'hack':
        output = [
          ...
          'Bypassing ICE... [SUCCESS]',
          'Downloading Arasaka Database... [||||||||||] 100%',
          'DATA_SECURE.',
        ];
        break;
      case '':
  ...
  return (
    <div
      className="h-full p-6 font-mono text-sm overflow-auto bg-black custom-scrollbar"
      style={{ color: COLORS.BLUE }}
      onClick={() => document.getElementById('terminal-input')?.focus()}
    >
      {history.map((line, i) => (
        <div key={i} className="mb-1 break-all">
          {line.startsWith('netrunner') ? (
            <span className="text-[var(--color-blue)]">{line}</span>
          ) : line.includes('[SUCCESS]') ? (
            <span style={{ color: COLORS.YELLOW }}>{line}</span>
