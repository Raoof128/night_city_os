const SettingsApp = ({ config, onUpdateConfig, auditLog }) => {
  return (
    <div className="p-8 space-y-8 font-mono text-[var(--color-blue)]">
      <section>
        <h3 className="text-[var(--color-yellow)] text-xl font-bold mb-4 border-b border-gray-800 pb-2">
          DISPLAY_CONFIG
        </h3>
      ...
      <section>
        ...
        <div className="flex items-center gap-4 border border-gray-800 p-4">
          <div className="w-16 h-16 bg-[var(--color-red)] flex items-center justify-center font-black text-2xl text-black">
            R
          </div>
          <div>
      ...
      <section>
        <div className="flex justify-between items-center mb-4 border-b border-gray-800 pb-2">
          ...
          <span className="text-[10px] bg-red-900/40 text-red-200 px-2 py-1 rounded">
            RESTRICTED_ACCESS
          </span>
        </div>
        <div className="h-64 overflow-y-auto custom-scrollbar border border-red-900/30 bg-black/40 p-2 font-mono text-xs">
          {(auditLog || []).map((log) => (
            <div
              key={log.id}
              className="grid grid-cols-[80px_1fr] gap-2 mb-1 hover:bg-white/5 p-1 rounded"
            >
              <span className="text-gray-500">{log.time}</span>
              <span
                className={`${log.type === 'warning' ? 'text-[var(--color-red)]' : 'text-[var(--color-blue)]'}`}
              >
