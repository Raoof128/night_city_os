...
const AgentCard = ({
  ...
  return (
    <div
      className={`bg-black/80 p-4 border border-gray-800 rounded-lg relative overflow-hidden transition-all duration-300 ${isRunning ? 'border-[var(--color-yellow)] shadow-[0_0_15px_rgba(255,224,0,0.2)]' : 'hover:border-[var(--color-blue)]'} ${busy ? 'opacity-50' : ''}`}
    >
      {requiresAttention && (
      ...
      <div className="flex items-start gap-4">
        <div
          className={`w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-full bg-gray-900 border-2 ${isRunning ? 'border-[var(--color-yellow)]' : 'border-gray-700'}`}
        >
          <Icon
            size={24}
            className={
              isRunning ? 'text-[var(--color-yellow)] animate-pulse' : 'text-[var(--color-blue)]'
            }
          />
        </div>
  ...
  const runAgent = (agentName) => {
    ...
    setTimeout(() => {
      ...
      if (agentName === 'negotiate') {
        ...
        if (billToNegotiate) {
          const negotiatedBills = data.upcomingBills.map((b, i) =>
            i === 0 ? { ...b, amount: b.amount * 0.9 } : b
          );
      ...
      if (agentName === 'debt') {
        ...
        if (debtToPay) {
          const paidDebt = data.debts.map((d, i) =>
            i === 0 ? { ...d, amount: d.amount * 0.95 } : d
          );
          onUpdateData({
            ...data,
            debts: paidDebt,
            balance: data.balance - debtToPay.amount * 0.05,
          });
      ...
      if (agentName === 'portfolio') {
        onUpdateData({
          ...data,
          portfolio: { ...data.portfolio, currentAllocation: data.portfolio.targetAllocation },
        });
