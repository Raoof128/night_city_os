...
const ToggleSwitch = ({ enabled }) => (
    <div className={`w-10 h-5 flex items-center rounded-full px-1 cursor-pointer ${enabled ? 'bg-[var(--color-green)]' : 'bg-gray-700'}`}>
        <div className={`w-3 h-3 rounded-full bg-white transform transition-transform ${enabled ? 'translate-x-5' : ''}`} />
    </div>
);
...
export default function Security() {
    return (
        <motion.div
            ...
            className="space-y-8"
        >
            <h2 className="text-2xl font-black text-[var(--color-yellow)] tracking-tighter">CYBERSECURITY & PRIVACY</h2>
            ...
            <Section title="PRIVACY_FEATURES">
                ...
                <SecuritySetting
                    ...
                    control={<button className="text-xs bg-[var(--color-blue)] hover:bg-[var(--color-blue)]/80 text-white font-bold py-2 px-3 rounded-md">EXPORT</button>}
                />
                <SecuritySetting
                    icon={Timer}
                    title="Delete Account"
                    description="Permanently erase all your data."
                    control={<button className="text-xs bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-3 rounded-md">DELETE</button>}
                />
            </Section>
        </motion.div>
    );
