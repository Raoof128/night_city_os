  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setFinalInsight({
        ...
        content:
          "Detected 15% increase in 'Night Market' spending. Reallocating 200 €$ from 'Entertainment' could maximize your cyberware savings goal by next month.",
      });
    }, delay + 500);
  }, []);
      ...
      <div className="space-y-1">
        {thoughtSteps.map((step, i) => (
          ...
          >
            ...
            <span>{step}</span>
            {i === thoughtSteps.length - 1 && !finalInsight && (
      ...
      {finalInsight && (
        ...
        >
          ...
          <div className="mt-3 flex gap-2">
            <button className="px-3 py-1 bg-[var(--color-yellow)] text-black text-xs font-bold hover:bg-white transition-colors">
              APPLY_LIMITS
            </button>
            <button className="px-3 py-1 border border-gray-600 text-gray-400 text-xs font-bold hover:text-white transition-colors">
              DISMISS
            </button>
          </div>
        </motion.div>
...
const MonteCarloSim = ({ currentBalance, monthlyBurn }) => {
    ...
    for (let i = 0; i <= months; i++) {
      ...
      if (i > 0) {
        ...
        scenarios
          ...
          .forEach((s) => (simBal += s.impact));
      }
      // One-time events at month 3 (simulation)
      if (i === 3) {
        scenarios
          .filter((s) => s.active && s.type === 'onetime')
          .forEach((s) => (simBal += s.impact));
      }
  ...
  const maxY = Math.max(
    ...chartData.simPoints.map((p) => p.y),
    ...chartData.basePoints.map((p) => p.y),
    currentBalance
  );
  const minY = Math.min(
    ...chartData.simPoints.map((p) => p.y),
  ...
  return (
    <div className="flex gap-6 h-full">
      <div className="w-1/3 space-y-4">
        <h3 className="text-[var(--color-yellow)] font-bold text-sm tracking-widest border-b border-gray-800 pb-2">
          SCENARIO_BUILDER
        </h3>
        <div className="space-y-2">
          {scenarios.map((s) => (
            <div
              ...
              className={`p-3 border cursor-pointer transition-all flex justify-between items-center ${s.active ? 'border-[var(--color-yellow)] bg-[var(--color-yellow)]/10' : 'border-gray-700 bg-black/40 hover:border-gray-500'}`}
            >
              <div>
                <div className={`text-xs font-bold ${s.active ? 'text-white' : 'text-gray-500'}`}>
                  {s.name}
                </div>
                <div className="text-[10px] font-mono text-gray-500">
                  {s.type === 'monthly' ? '/MO' : 'ONCE'}
                </div>
              </div>
              <div
                className={`font-mono text-xs ${s.impact > 0 ? 'text-[var(--color-blue)]' : 'text-[var(--color-red)]'}`}
              >
                {s.impact > 0 ? '+' : ''}
                {s.impact.toLocaleString()}
              </div>
            </div>
          ))}
        </div>
        <div className="p-3 bg-white/5 text-xs text-gray-400 font-mono">
          PROJECTION:{' '}
          {chartData.simPoints[months].y > 0 ? (
            <span className="text-[var(--color-blue)]">SOLVENT</span>
          ) : (
            <span className="text-[var(--color-red)]">BANKRUPT</span>
          )}
          <br />
          DELTA: {(
            chartData.simPoints[months].y - chartData.basePoints[months].y
          ).toLocaleString()}{' '}
          €$
        </div>
      </div>
      ...
      <div className="flex-1 bg-black/40 border border-gray-800 relative p-4 flex flex-col">
        <h3 className="text-gray-500 font-bold text-xs absolute top-2 left-2">
          NET_WORTH_PROJECTION (12 MO)
        </h3>
        <div className="flex-1 relative mt-6">
          ...
          <svg className="w-full h-full overflow-visible" preserveAspectRatio="none">
            ...
            {/* Base Line (Gray) */}
            <polyline
              points={chartData.basePoints
                .map((p, i) => `${(i / months) * 100},${normalize(p.y)}`)
                .join(' ')}
              fill="none"
              stroke="#555"
              strokeWidth="2"
            ...
            <motion.path
              d={
                `M 0,${normalize(chartData.simPoints[0].y)} ` +
                chartData.simPoints
                  .map((p, i) => `L ${(i / months) * 100},${normalize(p.y)}`)
                  .join(' ') +
                ` L 100,100 L 0,100 Z`
              }
          ...
          <div
            className="absolute top-0 bottom-0 border-l border-[var(--color-blue)] border-dashed opacity-50 flex items-end pb-1"
            style={{ left: `${(3 / months) * 100}%` }}
          >
            <span className="text-[10px] text-[var(--color-blue)] transform -translate-x-1/2 bg-black px-1">
              EVENT_HORIZON
            </span>
          </div>
        </div>
  ...
  const categoryData = useMemo(() => {
    ...
    if (data.recent && Array.isArray(data.recent)) {
      data.recent.forEach((tx) => {
        ...
        const key =
          Object.keys(defaultCats).find((k) => k.toLowerCase() === cat.toLowerCase()) || 'Other';
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-800">
        <div className="flex items-center gap-2 text-[var(--color-yellow)]">
          ...
          <h2 className="text-lg font-black tracking-widest">
            FINANCE_OS <span className="text-xs text-gray-600 align-top">v2.5</span>
          </h2>
        </div>
                ...
                <StatCard
                  ...
                  value={
                    <span className="privacy-blur">
                      {(data.balance / (data.spent || 1)).toFixed(1)} MO
                    </span>
                  }
              ...
              <div className="border border-gray-800 bg-[var(--color-surface)]/30">
                <div className="p-3 border-b border-gray-800 flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => {
                        const csvContent =
                          ...
                          data.recent
                            .map((e) => `${e.time},${e.desc},${e.category},${e.amount}`)
                            .join('\n');
                    ...
                    <button
                      className="text-[var(--color-yellow)] text-xs font-bold hover:underline"
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-8 h-8 rounded-full bg-gray-900 flex items-center justify-center border border-gray-700 group-hover:border-[var(--color-yellow)]">
                          <Zap
                            size={14}
                            className="text-gray-400 group-hover:text-[var(--color-yellow)]"
                          />
                        </div>
                        <div>
                          <div className="font-bold text-sm text-gray-200 group-hover:text-white flex items-center gap-2">
                            {tx.enriched && tx.logo && (
                              <img src={tx.logo} alt="" className="w-4 h-4" />
                            )}
                            {tx.desc}
                            {tx.isTaxDeductible && (
                              <span className="text-[9px] bg-[var(--color-blue)] text-black px-1 rounded font-black">
                                TAX
                              </span>
                            )}
                          </div>
                          <div className="text-xs text-gray-600 font-mono">
                            {tx.time} <span className="text-gray-500 mx-1">{'//'}</span>{' '}
                      ...
                      <div className="flex items-center gap-4">
                        <span
                          className={`text-xs font-bold ${tx.status === 'Pending' ? 'text-gray-500' : 'text-white'}`}
                        >
                          {tx.status || 'Posted'}
                        </span>
                        <button
                          onClick={() => alert('Splitting transaction... (mocked)')}
                          className="text-xs font-bold text-[var(--color-blue)] hover:underline"
                        >
                          Split
                        </button>
                        <span className="font-mono font-bold text-[var(--color-red)] privacy-blur">
                          -{convert(tx.amount)}
                        </span>
                      </div>
                    </div>
                ...
                <div className="divide-y divide-gray-800">
                  {transactions
                    .filter(
                      (tx) =>
                        (filter.description === '' ||
                          tx.desc.toLowerCase().includes(filter.description.toLowerCase())) &&
          ...
          {activeTab === 'spending' && (
            ...
            >
              <div className="flex-1 bg-[var(--color-surface)]/30 border border-gray-800 p-6 flex flex-col items-center">
                <h3 className="text-xs font-bold text-gray-500 mb-6 uppercase tracking-widest">
          ...
          {activeTab === 'assets' && (
            ...
            >
              ...
              <div className="bg-[var(--color-surface)]/30 border border-gray-800 p-4">
                <h3 className="text-[var(--color-yellow)] font-bold text-sm tracking-widest mb-4 border-b border-gray-800 pb-2">
                  PHYSICAL_ASSETS
                </h3>
                <div className="space-y-2">
                  {(data.assets || []).map((asset) => (
                    <div
                      key={asset.id}
                      className="flex justify-between items-center p-3 bg-black/40 border border-gray-700 hover:border-[var(--color-blue)] transition-colors group"
                    >
                      <div>
                        <div className="text-sm font-bold text-white group-hover:text-[var(--color-blue)]">
                          {asset.name}
                        </div>
                      ...
                      <div className="text-right">
                        <div className="text-sm font-mono font-bold text-white privacy-blur">
                          {convert(asset.value)}
                        </div>
                  ...
                  <button
                    className="w-full py-2 border border-dashed border-gray-600 text-gray-400 text-xs font-bold hover:border-[var(--color-yellow)] hover:text-[var(--color-yellow)] transition-colors"
                ...
                {showAddAssetForm && (
                  <div className="mt-4 p-4 border border-gray-700">
                    ...
                    <div className="flex flex-col gap-2">
                      <input
                        type="text"
                        placeholder="Asset Name (e.g., Sneakers)"
                        className="bg-black border border-gray-700 text-xs p-2 text-white outline-none focus:border-[var(--color-blue)]"
                      />
                      <input
                        type="text"
                        placeholder="Asset Type (e.g., Collectible)"
                        className="bg-black border border-gray-700 text-xs p-2 text-white outline-none focus:border-[var(--color-blue)]"
                      />
                      <input
                        type="number"
                      ...
                      />
                      <button
                        className="bg-[var(--color-blue)]/20 text-[var(--color-blue)] border border-[var(--color-blue)] px-2 py-1 text-xs font-bold hover:bg-[var(--color-blue)] hover:text-black transition-colors"
                        onClick={() => alert('Adding new asset... (mocked)')}
                      >
                        Add Asset
                      </button>
                    </div>
              ...
              <div className="space-y-4">
                <div className="bg-[var(--color-surface)]/30 border border-gray-800 p-4">
                  <h3 className="text-[var(--color-red)] font-bold text-sm tracking-widest mb-4 border-b border-gray-800 pb-2">
                    RECURRING_DRAINS
                  </h3>
                  {!data.subscriptions || data.subscriptions.length === 0 ? (
                    <div className="text-center text-gray-500 text-xs py-8">
                      NO_SUBSCRIPTIONS_DETECTED
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {data.subscriptions.map((sub, i) => (
                        <div
                          key={i}
                          className="flex justify-between items-center p-2 bg-black/20 border-l-2 border-[var(--color-red)]"
                        >
                          <div className="flex items-center gap-2">
                            <Zap size={12} className="text-[var(--color-red)]" />
                            <span className="text-xs font-bold text-gray-300">{sub.service}</span>
                          </div>
                          <div className="font-mono text-xs text-white privacy-blur">
                            {convert(sub.amount)} / {sub.cycle}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {/* Smart Rule Feedback */}
                <div className="bg-[var(--color-surface)]/30 border border-gray-800 p-4">
                  <h3 className="text-[var(--color-blue)] font-bold text-sm tracking-widest mb-2 border-b border-gray-800 pb-2">
                    NEURAL_TRAINING
                  </h3>
                  <div className="flex gap-2">
                    <input
