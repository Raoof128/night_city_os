...
const OptimizationBot = ({
  ...
  return (
    <div
      className={`bg-gray-900/50 p-4 border border-gray-800 rounded-lg transition-all duration-300 hover:border-[var(--color-blue)] ${busy ? 'opacity-50' : ''}`}
    >
        ...
        <div className="flex-grow">
          ...
          {lastRunResult && (
            <div className="mt-2 text-xs font-mono p-2 bg-black/50 border-l-2 border-[var(--color-blue)]">
              <span className="font-bold text-[var(--color-blue)]">LAST_RUN:</span>{' '}
              <span className="text-gray-300">{lastRunResult}</span>
            </div>
          )}
        </div>
        <button
    ...
    <div className="p-4 bg-transparent text-white h-full mt-8">
      ...
      <div className="space-y-4">
        <OptimizationBot
          ...
          actionText="SCAN"
          onAction={() =>
            handleAction('Redundancy', 'SCANNING... Found redundant subscription: Spotify.')
          }
