        ...
        <div className="space-y-2">
            {data.map(item => (
                <div key={item.label} className="flex items-center justify-between text-xs font-mono">
                    ...
                    <span className={item.type === 'asset' ? 'text-[var(--color-blue)]' : 'text-[var(--color-red)]'}>
...
export default function Forecasting() {
    return (
        <motion.div
            ...
            className="space-y-8"
        >
            <h2 className="text-2xl font-black text-[var(--color-yellow)] tracking-tighter">ADVANCED_ANALYTICS & FORECASTING</h2>

            {/* Financial Health Tracking */}
            <div>
                <h3 className="text-sm font-bold text-white border-b border-[var(--color-blue)]/30 pb-2 mb-4 tracking-widest">FINANCIAL_HEALTH_TRACKING</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                     <AnalyticsCard icon={TrendingUp} title="Net Worth Waterfall" color="text-[var(--color-yellow)]">
                        <NetWorthWaterfall />
                    </AnalyticsCard>
                    <AnalyticsCard icon={Gauge} title="Financial Health Score" color="text-[var(--color-blue)]">
                        <HealthScoreGauge score={78} />
                    </AnalyticsCard>
                     <AnalyticsCard icon={Flame} title="Burn Rate & Runway" value="9,800 €$/mo" subtext="12.7 months runway" color="text-[var(--color-red)]" />
                     <AnalyticsCard icon={PiggyBank} title="Savings Rate" value="22%" subtext="vs 15% target" color="text-[var(--color-green)]" />
                </div>
            </div>

            {/* Predictive Intelligence */}
            <div>
                 <h3 className="text-sm font-bold text-white border-b border-[var(--color-blue)]/30 pb-2 mb-4 tracking-widest">PREDICTIVE_INTELLIGENCE</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <AnalyticsCard icon={BarChart} title="Cash Flow Forecasting (6 mo)" color="text-[var(--color-blue)]" subtext="Projection based on historical patterns">
                        <CashFlowChart />
                    </AnalyticsCard>
                    <AnalyticsCard icon={GanttChartSquare} title="Monte Carlo Simulations" color="text-[var(--color-yellow)]" subtext="Test market shock scenarios">
                         <div className="text-center text-gray-500 text-sm pt-4">WHAT_IF_CONTROLS</div>
                    </AnalyticsCard>
                    <AnalyticsCard icon={AreaChart} title="Income Volatility" color="text-[var(--color-green)]" subtext="Lean month prediction for freelancers">
                         <div className="text-center text-gray-500 text-sm pt-4">VOLATILITY_WAVEFORM</div>
                    </AnalyticsCard>
                    <AnalyticsCard icon={TrendingUp} title="Expense Trend Detection" color="text-[var(--color-red)]">
                        <ExpenseTrendChart />
                    </AnalyticsCard>
                </div>
            </div>
            ...
            <div>
                 <h3 className="text-sm font-bold text-white border-b border-[var(--color-blue)]/30 pb-2 mb-4 tracking-widest">VISUALIZATION_ARSENAL</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <AnalyticsCard icon={GitCommit} title="Cash Flow Sankey" color="text-[var(--color-yellow)]">
                        <SankeyPlaceholder />
                    </AnalyticsCard>
                    <AnalyticsCard icon={AreaChart} title="Net Worth Area Chart" color="text-[var(--color-green)]">
